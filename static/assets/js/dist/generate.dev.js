...
$("#generateData").click(function () {
  ...
  if (vechileNmub >= 0 && vechileNmub <= 200 && vechileNmub != '' && stationList.length >= 1) {
    $("#multi-filter-select").dataTable().fnDestroy(); // 销毁原有表格，生成新的表格
    ...
    if (yes) {
      ...
      if (numofchargingpiles >= 0 && numofchargingpiles <= 200 && numofchargingpiles != '') {
        ...
        marker.setLabel({
          ...
          //设置文本标注偏移量
          content: "<div class='info'>" + "A" + lnglatpair[0] + ":(" + lnglatpair[1] + "," + lnglatpair[2] + ") " + numofchargingpiles + "</div>",
          //设置文本标注内容
